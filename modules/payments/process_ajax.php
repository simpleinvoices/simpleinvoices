...
while ($invoice = getInvoices($sth)) {
	...
	if (strpos(strtolower($invoice['index_id']), $q) !== false) {
		...
		echo "$invoice['id']|<table><tr><td class='details_screen'></td><td>$invoice['index_name']</td><td  class='details_screen'>Total: </td><td>$invoice['total'] </td></tr><tr><td class='details_screen'>Biller: </td><td>$invoice['biller'] </td><td class='details_screen'>Paid: </td><td>$invoice['paid'] </td></tr><tr><td class='details_screen'>Customer: </td><td>$invoice['customer'] </td><td class='details_screen'>Owing: </td><td><u>$invoice['owing']</u></td></tr></table>\n";
	}
}
