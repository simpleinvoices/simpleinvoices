...
$logger->log('Paypal API page called', LegacyLogger::INFO);
if ($p->validate_ipn()) {
#if (!empty($_POST)) {

	$logger->log('Paypal validate success', LegacyLogger::INFO);

	//insert into payments
	$paypal_data ="";
	foreach ($p->ipn_data as $key => $value) { $paypal_data .= "\n$key: $value"; }
	$logger->log('Paypal Data:', LegacyLogger::INFO);
	...
	$logger->log('Paypal - custom='.$_POST['custom'],LegacyLogger::INFO);
	foreach ($custom_array as $key => $value)
	{
		if( strstr($value,"domain_id:"))
		{
			$logger->log('Paypal - value='.$value,LegacyLogger::INFO);
			$domain_id = substr($value, 10);
			#$domain_id = substr($domain_id, 0, -1);
		}
	}

	$logger->log('Paypal - domain_id='.$domain_id.'EOM', LegacyLogger::INFO);
	...
	$logger->log('Paypal - number of times this payment is in the db: '.$number_of_payments, LegacyLogger::INFO);
	...
	$logger->log('Paypal validate failed', LegacyLogger::INFO);
}
