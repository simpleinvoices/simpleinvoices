...
if ($extension_id == null) {	// extension not yet registered
  ...
  $sth = dbQuery($sql,':id',$extension_id, ':domain_id', $auth_session->domain_id);
  $info = $sth->fetchAll(PDO::FETCH_ASSOC);

  $extension_name = $info[0]['name'];
  $extension_desc = $info[0]['description'];
  
  $sql = "SELECT * FROM ".TB_PREFIX."system_defaults WHERE extension_id = :id AND domain_id = :domain_id;";
  $sth = dbQuery($sql,':id',$extension_id, ':domain_id', $auth_session->domain_id);
  $info = $sth->fetchAll(PDO::FETCH_ASSOC);
  $count = count($info);
}
