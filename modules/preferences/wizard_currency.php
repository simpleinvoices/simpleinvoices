...
if (($_POST['op'] ?? '') === 'wizard_currency_sign') {
	...
	if ($preference) {
		...
		$sql = 'UPDATE ' . TB_PREFIX . 'preferences SET pref_currency_sign = :sign WHERE pref_id = :id AND domain_id = :domain_id';
		if (dbQuery($sql, ':sign', $sign, ':id', $pref_id, ':domain_id', $auth_session->domain_id)) {
			$saved = true;
