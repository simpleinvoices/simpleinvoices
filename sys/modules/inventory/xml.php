...
$count = $sth_count_rows;
	...
	foreach ($inventory_all as $row) {
		$xml .= "<row id='".$row['id']."'>";
		$xml .= "<cell><![CDATA[
		<a class='index_table' title='$LANG[view] ".$row['name']."' href='index.php?module=inventory&view=view&id=$row[id]'><img src='" . $baseUrl . "sys/images/common/view.png' height='16' border='-5px' padding='-4px' valign='bottom' /></a>
		<a class='index_table' title='$LANG[edit] ".$row['name']."' href='index.php?module=inventory&view=edit&id=$row[id]'><img src='" . $baseUrl . "sys/images/common/edit.png' height='16' border='-5px' padding='-4px' valign='bottom' /></a>
		]]></cell>";
