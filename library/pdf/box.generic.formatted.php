...
class GenericFormattedBox extends GenericBox {
    ...
    public function _get_collapsable_top_margin_internal() {
        $positive_margin = 0;
        $negative_margin = 0;
        ...
        if ($border->top->get_width() > 0 || $padding->top->value > 0) {
            return 0;
        }

        while (!is_null($current_box) && $current_box->isBlockLevel()) {
    ...
    public function _get_collapsable_top_margin_external() {
        $positive_margin = 0;
        $negative_margin = 0;
    ...
    public function _get_collapsable_bottom_margin_external() {
        $positive_margin = 0;
        $negative_margin = 0;
        ...
        while (!is_null($current_box) && $current_box->isBlockLevel()) {
            ...
            if ($border->bottom->get_width() > 0 || $padding->bottom->value > 0) {
                $current_box = null;
            } else {
    ...
    public function collapse_margin_bottom(&$parent, &$context) {
        /**
         ...
         *
         * @link http://www.w3.org/TR/CSS21/visudet.html#Computing_widths_and_margins CSS 2.1 8.3.1 Calculating widths
         *       and margins
         */
        ...
        if (!is_null($last) && $is_last && // This element is a last in-flow block level element AND
         ...
         */
        if ($is_last && ($parent_border->bottom->get_width() > 0 || $parent_padding->bottom->value > 0)) {
    ...
    public function collapse_margin(&$parent, &$context) {
    ...
    public function readCSS(&$state) {
        ...
        $this->_readCSS($state,
                        array(CSS_OVERFLOW, CSS_PAGE_BREAK_AFTER, CSS_PAGE_BREAK_BEFORE, CSS_PAGE_BREAK_INSIDE,
                                        CSS_ORPHANS, CSS_WIDOWS, CSS_POSITION, CSS_TEXT_ALIGN, CSS_WHITE_SPACE,
                                        CSS_CLEAR, CSS_CONTENT, CSS_HTML2PS_PSEUDOELEMENTS, CSS_FLOAT, CSS_Z_INDEX,
                                        CSS_HTML2PS_ALIGN, CSS_HTML2PS_NOWRAP, CSS_DIRECTION, CSS_PAGE));

        $this->_readCSSLengths($state,
                        array(CSS_BACKGROUND, CSS_BORDER, CSS_BOTTOM, CSS_TOP, CSS_LEFT, CSS_RIGHT, CSS_MARGIN,
                              CSS_PADDING, CSS_TEXT_INDENT, CSS_HTML2PS_COMPOSITE_WIDTH, CSS_HEIGHT,
                              CSS_MIN_HEIGHT, CSS_MAX_HEIGHT, CSS_LETTER_SPACING));
    ...
    public function _calc_percentage_margins(&$parent) {
        $margin = $this->getCSSProperty(CSS_MARGIN);
    ...
    public function _calc_percentage_padding(&$parent) {
     ...
     */
    public function _calc_percentage_width(&$parent, &$context) {
        $wc = $this->getCSSProperty(CSS_WIDTH);
        if ($wc->isFraction()) {
            $containing_block = & $this->_get_containing_block();
    ...
    public function _calc_auto_width_margins(&$parent) {
    ...
    public function _calc_auto_width_margins_float(&$parent) {
    ...
    function _calc_auto_width_margins_normal(&$parent) {
        ...
        // If 'width' is set to 'auto', any other 'auto' values become '0' and 'width' follows from the resulting
        // equality.
    ...
    function _get_vert_extra() {
        return $this->get_extra_top() + $this->get_extra_bottom();
    }

    function _get_hor_extra() {
        return $this->get_extra_left() + $this->get_extra_right();
    }
    ...
    function &_get_containing_block_absolute() {
        ...
        if (is_null($parent)) {
            trigger_error("No containing block found for absolute-positioned element", E_USER_ERROR);
        }
        ...
        while ((!is_null($parent->parent)) && ($parent->getCSSProperty(CSS_POSITION) === POSITION_STATIC)) {
    ...
    function &_get_containing_block_static() {
        ...
        while (!is_null($parent->parent) && !$parent->isBlockLevel() && !$parent->isCell()) {
            $parent = & $parent->parent;
        }
    ...
    function get_extra_left() {
        ...
        return $this->get_margin_left() + $left_border->get_width() + $this->get_padding_left();
    }

    function get_extra_right() {
        $border = $this->getCSSProperty(CSS_BORDER);
        $right_border = $border->right;
        return $this->get_margin_right() + $right_border->get_width() + $this->get_padding_right();
    }

    function get_extra_top() {
        $border = $this->getCSSProperty(CSS_BORDER);
        return $this->get_margin_top() + $border->top->get_width() + $this->get_padding_top();
    }
    ...
    function get_left_border() {
        ...
        return $this->get_left() - $padding->left->value - $border->left->get_width();
    }
    ...
    function get_right_border() {
        ...
        return $this->get_left() + $this->get_width() + $padding->right->value + $border->right->get_width();
    }

    function get_top_border() {
        $border = $this->getCSSProperty(CSS_BORDER);

        return $this->get_top_padding() + $border->top->get_width();
    }
    ...
    function get_top_padding() {
        ...
        return $this->get_top() + $padding->top->value;
    }
    ...
    function get_left_margin() {
        return $this->get_left() - $this->get_extra_left();
    }

    function get_right_margin() {
        return $this->get_right() + $this->get_extra_right();
    }

    function get_bottom_margin() {
        return $this->get_bottom() - $this->get_extra_bottom();
    }

    function get_top_margin() {
        $margin = $this->getCSSProperty(CSS_MARGIN);

        return $this->get_top_border() + $margin->top->value;
    }
    ...
    function contains_point_margin($x, $y) {
        ...
        $eps = 0.1;
        return $this->get_left_margin() - $eps <= $x && $this->get_right_margin() + $eps >= $x &&
                         $this->get_top_margin() + $eps >= $y && $this->get_bottom_margin() < $y;
    }
    ...
    function get_full_height() {
        return $this->get_height() + $this->get_extra_top() + $this->get_extra_bottom();
    }
    ...
    function out_of_flow() {
        ...
        return $position == POSITION_ABSOLUTE || $position == POSITION_FIXED || $display == 'none';
    }
    ...
    function line_break_allowed() {
        ...
        return ($white_space === WHITESPACE_NORMAL || $white_space === WHITESPACE_PRE_WRAP ||
                         $white_space === WHITESPACE_PRE_LINE) && $nowrap === NOWRAP_NORMAL;
    }
    ...
    function isVisibleInFlow() {
        ...
        return $visibility === VISIBILITY_VISIBLE && $position !== POSITION_FIXED;
    }
     ...
     * element(s) up or down without changing their size.
     * 'top' moves
    ...
    function offsetRelative() {
        ...
        $this->offset($horizontal_offset, $vertical_offset);
    }
}
