...
class BackgroundImage {
     ...
     */
    public function __construct($url, $image) {
        $this->_url = $url;
        $this->_image = $image;
    }
    ...
    /**
     * Checks if this value is equivalent to default value.
     * According to CSS2, default value
     ...
     * @uses OutputDriver
     */
    public function show(&$driver, $box, $repeat, $position, $attachment) {
        /**
        ...
        if (!$GLOBALS['g_config']['debugnoclip']) {
             ...
             * Note that background image is drawn in the padding
         ...
         * current output driver's tiled image output functions.
         * Note that px2pt(1) is an image scaling factor; as all
         ...
         *
         * @link file:///C:/docs/css/colors.html#propdef-background-repeat CSS 2.1 'background-repeat' property
         *       description
         */
        switch ($repeat) {
            case BR_NO_REPEAT:
            ...
            case BR_REPEAT_X:
                 ...
                 */
                $driver->image_rx($this->_image, $left + $x_offset, $top - $image_height - $y_offset, $image_width,
                                $right, $x_offset, $y_offset, px2pt(1));
                break;
            case BR_REPEAT_Y:
                /**
                 * 'background-repeat: repeat-y' case; vertical tiling
                 */
                $driver->image_ry($this->_image, $left + $x_offset, $top - $image_height - $y_offset, $image_height,
                                $bottom, $x_offset, $y_offset, px2pt(1));
                break;
            case BR_REPEAT:
                /**
                 * 'background-repeat: repeat' case; full tiling
                 */
                $driver->image_rx_ry($this->_image, $left + $x_offset, $top - $image_height + $y_offset, $image_width,
                                $image_height, $right, $bottom, $x_offset, $y_offset, px2pt(1));
                break;
        }
