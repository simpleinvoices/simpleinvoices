...
// RCR - 20160708 - Made a class constant as only place used.
//define('HTTP_OK', 200);
...
class FetcherUrl extends Fetcher {
    ...
    public function get_data($data_id) {
        ...
        if ($this->fetch($data_id)) {
            if ($this->code != self::HTTP_OK) {
                ...
                error_log("FetcherUrl - Cannot open $data_id, HTTP result code is: ".$this->code);
    ...
    private function _extract_code($res) {
        // Check return code. Note the return code will always be contained in
        // the response, so the we may not check the result of 'preg_match' -
        // it matches always.
    ...
    private function _fix_location($location) {
    ...
    private function _get() {
        ...
        // Build the HEAD request header (we're saying we're just a browser as some
        // pages don't like non-standard user-agents)
    ...
    private function _process_code($res, $used_get = false) {
        $matches = null;
        switch ($this->code) {
            case '200': // OK
                if (preg_match('/(.*?)\r\n\r\n(.*)/s', $res, $matches)) {
            ...
            case '405':
                // Method not allowed; some sites (like MSN.COM) do not like "HEAD" HTTP requests
    ...
    private function _header_basic_authorization() {
