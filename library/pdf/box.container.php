...
class GenericContainerBox extends GenericFormattedBox {
     ...
     * for example: "<div>text<div style='float:left'>float</div>word</div>".
     * In
    ...
    public function show(&$driver) {
        ...
        /**
         * Sometimes the content may overflow container boxes.
         * This situation arise, for example,
        ...
        for ($i = 0, $size = count($this->content); $i < $size; $i++) {
             ...
             * so every box will pass this check.
             * The alternative is to add this check into every
            ...
            if ($child->isVisibleInFlow()) {
                 ...
                 * belongs to current output page.
                 * If not, there will be no reason to draw this
                 * child this time.
                 *
    ...
    /**
     * Render current fixed-positioned container box using the specified output method.
     * Unlike
    ...
    //
    public function insert_child($index, &$box) {
        $box->parent = & $this;
    ...
    public function insert_before(&$what, &$where) {
        if ($where) {
    ...
    public function add_child(&$box) {
        $this->append_child($box);
    }

    public function append_child(&$box) {
    ...
    public function offset_if_first(&$box, $dx, $dy) {
    ...
    public function add_deferred_float(&$float) {
     ...
     */
    public function create_content(&$root, &$pipeline) {
        // Initialize content
    ...
    public function get_content_callback(&$node) {
    ...
    public function get_real_full_height() {
        ...
        for ($i = 0; $i < $content_size; $i++) {
            if (!$this->content[$i]->is_null()) {
                ...
                if (!$this->content[$i]->is_container()) {
                    $min_bottom = min($min_bottom, $this->content[$i]->get_bottom_margin());
                } else {
                    ...
                    if ($content_overflow == OVERFLOW_HIDDEN) {
                        ...
                        $min_bottom = min($min_bottom, $this->content[$i]->get_bottom_margin(),
                                        $this->content[$i]->get_top_margin() - $this->content[$i]->get_real_full_height());
                    }
                }
            }
        }
    ...
    public function _line_length_delta(&$context) {
    ...
    public function get_min_width_natural(&$context) {
        ...
        if ($start_index < $content_size) {
            $ti = $this->getCSSProperty(CSS_TEXT_INDENT);
            $minw = $ti->calculate($this) + $this->content[$start_index]->get_min_width_natural($context);
        } else {
            $minw = 0;
        }
        ...
        /**
         * Apply width constraint to min width.
         * Return maximal value
         */
        /* Commented out 20160708 RCR as not used.
    ...
    public function get_min_nowrap_width(&$context) {
    ...
    public function close_line(&$context, $lastline = false) {
        ...
        // first, we need to aling all baseline-aligned boxes to determine the basic line-box height, top and bottom
        // edges
        ...
        for ($i = 0; $i < $size_baselined; $i++) {
            ...
            $height = max($height, $baselined[$i]->get_full_height() + $baselined[$i]->getBaselineOffset(),
        ...
        for ($i = 0; $i < $size; $i++) {
            ...
            if ($vertical_align == VA_SUB) {
                $this->_line[$i]->baseline = $baseline + $this->_line[$i]->get_full_height() / 2;
            }
        }
        ...
        for ($i = 0; $i < $size; $i++) {
            ...
            if ($vertical_align == VA_MIDDLE) {
                $this->_line[$i]->baseline = $this->_line[$i]->default_baseline +
                                 ($height / 2 - $this->_line[$i]->get_full_height() / 2);
            }
        }
        ...
        for ($i = 0; $i < $size; $i++) {
            $vertical_align = $this->_line[$i]->getCSSProperty(CSS_VERTICAL_ALIGN);
            if ($vertical_align == VA_BOTTOM) {
                $this->_line[$i]->baseline = $this->_line[$i]->default_baseline + $height -
                                 $this->_line[$i]->get_full_height();
            }
        }
        ...
        foreach ($this->_line as $line_element) {
            ...
            $effective_bottom = $line_element->get_top() - $line_element->get_height() -
    ...
    public function append_line(&$item) {
        $this->_line[] = & $item;
    }
    ...
    public function line_box_empty() {
        $size = count($this->_line);
        if ($size == 0) {
            return true;
        }
        ...
        // Scan line box
        for ($i = 0; $i < $size; $i++) {
            if (!is_whitespace($this->_line[$i]) && !$this->_line[$i]->is_null()) {
                return false;
            }
        }
    ...
    public function reflow_anchors(&$viewport, &$anchors, $page_heights) {
    ...
    public function fitFloats(&$context) {
     ...
     */
    public function reflow_static_float(&$parent, &$context) {
        ...
        // so _current_x value of container box will be still equal to ots left content edge; by calling float_left_x
        // again,
    ...
    public function remove_last_whitespace() {
        ...
        /*
         * Commented out as not used - RCR - 20160708
        $last = $this->content[$i];
         */
    ...
    //
    public function get_available_width(&$context) {
    ...
    public function _setupClip(&$driver) {
