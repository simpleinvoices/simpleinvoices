...
function ReadTTF($fontfile, $map) {
    ...
    // Here we use a hack; as, acording to OT specifications, when translated to ASCII,
    // these [...] strings must be identical; no longer than 63 characters; and
    // restricted to the printable ASCII subset, codes 33 through 126, except for the
    // 10 characters: '[', ']', '(', ')', '{', '}', '<', '>', '/', '%'. We can assume
    // that UCS-2 encoded string we receive can be easily translated to ASCII by removing
    // the high-byte of all two-byte characters
    // @formatter:off
...
function ReadAFM($file, $map) {
    if (!is_readable($file)) {
        // Logic should proceed to check for ttf file so don't
    ...
    foreach ($afm_lines as $l) {
        ...
        elseif ($code == 'FontBBox'          ) $fm['FontBBox']           = array($e[1], $e[2], $e[3], $e[4]);
    ...
    if (!isset($fm['FontName'])) {
        // RCR - Changed to not die 20160701
...
function MakeFontDescriptor($fm, $symbolic) {
    ...
    if (isset($fm['FontBBox'])) {
        ...
        // RCR - 20160630 Changed undefined $des to local variable $desc
...
function MakeWidthArray($fm) {
    ...
    $s = "array(\n\t";
// RCR 20160708 - Commented out unused variable $cw
//    $cw = $fm['Widths'];
    for ($i = 0; $i <= 255; $i++) {
        if (chr($i) == "'") {
            $s .= "'\\''";
...
function MakeFont($fontfile, $afmfile, $destdir, $destfile, $enc) {
    ...
    if ($fontfile) {
        ...
        if ($ext == 'ttf') {
            ...
            $str = "Error: unrecognized font file extension: $ext";
            error_log("MakeFont - $str");
            die($str);
        }
    ...
    if ($fontfile) {
        ...
        if (!file_exists($fontfile)) {
            $str = "Error: font file, $fontfile, not found.";
            error_log("MakeFont - $str");
            die($str);
        }
        ...
        if ($type == 'Type1') {
            ...
            if (!$pos) {
                $str = "Error: Font file, $file, does not seem to be valid Type1 (no 'eexec' found in it).";
                error_log("MakeFont - $str");
                die($str);
            }
            $size1 = $pos + 6;
            ...
            if (!$pos) {
                $str = "Error: Font file, $file, does not seem to be valid Type1 (no '00000000' found in it).";
                error_log("MakeFont - $str");
                die($str);
            }
