...
$logger->log('index.php:require_once (./extensions/'. $ext_name. '/include/init.php)', Zend_Log::INFO);//Matt
	}
}
...
$logger->log('index.php:include_once (./include/sql_patches.php)', Zend_Log::INFO);//Matt
	donePatches();
} else {
...
/*Matt*/
// if page referer is present, go to that page(module&view)
// ie. return to the last page visited before session timed-out
						if (isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER']) {
							$array = parseURLencoded($_SERVER['HTTP_REFERER']);			}
						$module 	= (isset($array['module']) && $array['module']) 	? $array['module'] 	: "invoices";
						$view 		= (isset($array['view']) && $array['view']) 		? $array['view'] 	: "manage";
/*^*/
					} else {
						$module 	= "index";
						$view 		= "index";
...
$logger->log('index.php:include_once (./extensions/'. $ext_name. '/modules/invoices/template.php)', Zend_Log::INFO);//Matt
...
$logger->log('index.php:include_once (' .$my_path. ')', Zend_Log::INFO);//Matt
	}
	exit(0);
}
...
$logger->log('index.php:include (./extensions/'. $ext_name. '/modules/'. $module. '/'. $view. '.php)', Zend_Log::INFO);//Matt
			$extensionXml++;
		}
	}
	...
	if ($extensionXml == 0 && $my_path = getCustomPath("$module/$view", 'module')) {
		include ($my_path);
$logger->log('index.php:include (' .$my_path. ')', Zend_Log::INFO);//Matt
	}
	exit(0);
}
...
//	echo '<!--menutpl='.print_r($menutpl,true).'lines='.print_r($lines,true).'sections='.print_r($sections,true).'-->';
