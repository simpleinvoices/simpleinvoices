...
for (var x = 0; x <= @endverbatim{{ $dynamic_line_items }}@verbatim; x++)
   {
...
{{-- <td>
					<select name="products{$smarty.section.line.index}">
						...
						@foreach(from=$matrix item=matrix_item)
							<option @if($product.id == $defaults.product) selected @endif value="{{ $matrix_item['id'] }}">{{ $matrix_item['display'] }}</option>
						@endforeach
					</select>
				</td>	
				<td>
