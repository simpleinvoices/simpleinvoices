...
// $domain_id is a parent class member
        ...
        for($i=0;$invoiceItem = $sth->fetch();$i++) {
            ...
            $tth = $db->query($sql, ':id', $invoiceItem['product_id'], ':domain_id', $domain_id);
...
// $invoiceItem['attribute_#'] (# = 1,2,3) was in old schema
			...
			$attr1 = $db->query($attr_sql, ':attr_id', $invoiceItem['attribute_1']);
            $invoiceItem['attr1'] = $attr1->fetch();
			$attr2 = $db->query($attr_sql, ':attr_id', $invoiceItem['attribute_2']);
            $invoiceItem['attr2'] = $attr2->fetch();
			$attr3 = $db->query($attr_sql, ':attr_id', $invoiceItem['attribute_3']);
