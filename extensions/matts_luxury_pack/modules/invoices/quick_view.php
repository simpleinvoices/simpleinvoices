...
if ($invoice['owing'] > 0 ) {
	...
	$url_for_pdf = "./index.php?module=export&view=pdf&id=" . (isset($invoice['id']) ? $invoice['id'] : 1);
...
for($i=1;$i<=4;$i++) {
	$customField[$i] = isset($invoice["custom_field$i"]) ? show_custom_field("invoice_cf$i",$invoice["custom_field$i"],"read",'summary', '','',5,':') : 1;
}
