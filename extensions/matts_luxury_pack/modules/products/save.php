...
#insert product
$saved = false;
if (class_exists('myproduct'))		$myproduct = new myproduct;

if ($op === 'insert_product') {
	
	if ($defaults['price_list']) {
		if ($id = $myproduct->insertNew()) // do insert_Product() if price_list enabled
			$saved = true;
...
//	echo "<script>alert('defaults=".print_r ($defaults,true).",defaults price_list=".$defaults['price_list']."|')</script>";
	...
	if ($defaults['price_list'] && $id = $myproduct->update()) {// do update_Product() if price_list enabled
		$saved = true;
		// saveCustomFieldValues($_POST['categorie'],lastInsertId());
	} else
