	...
	<div id="section-3" class="fragment">
		...
		<table width="100%" align="center">
			...
			<tr class="index_table">
				<td class="details_screen"><a href="index.php?module=invoices&amp;view=quick_view&id={{ $invoice['id'] ?? '' | urlencode }}">{{ $invoice['id'] ?? '' }}</a></td>
		...
		<table width="100%" align="center">
			...
			<tr class="index_table">
				<td class="details_screen">
                    <a href="index.php?module=customers&amp;view=details&action=edit&id={{ $sc['id'] ?? '' | urlencode }}">{{ $LANG['edit'] ?? '' }}</a>
                       :: 
                    <a href="index.php?module=customers&amp;view=details&action=view&id={{ $sc['id'] ?? '' | urlencode }}">{{ $LANG['view'] ?? '' }}</a>
                    </td>
				<td style="text-align:left" class="details_screen">{{ $sc['name'] ?? '' }}</td>
			</tr>
...
<table align="center">
	<tr>
		...
		<td><input type="text" name="name" value="{{ $customer['name'] ?? '' }}" size="50" id="name" class="validate[required]" /></td>
	</tr>
	<tr>
		<td class="details_screen">{{ $LANG['attention_short'] ?? '' }}
		<a
			 ...
			 />
				...
				<option value=''></option>
                            @foreach(from=$parent_customers item=c)
                                <option @if($c.id == $customer.parent_customer_id) selected @endif value="{{ $c['id'] ?? '' }}">{{ $c['name'] ?? '' }}</option>
                            @endforeach
                            </select>
                        @endif
                    </td>
