...
{{-- <tr>
	...
	<tr class='details_screen'>
		...
		<td colspan="2" class='details_screen align_right'>
			<a href='#' class="show-customer" @verbatim onclick="$('.customer').show(); $('.show-customer').hide(); @endverbatim"><img src="./images/common/magnifier_zoom_in.png" title="{{ $LANG['show_details'] ?? '' }}" alt=""/></a>
			<a href='#' class="customer" @verbatim onclick="$('.customer').hide(); $('.show-customer').show(); @endverbatim"><img src="./images/common/magnifier_zoom_out.png" title="{{ $LANG['hide_details'] ?? '' }}" alt="" /></a>
		</td>
	</tr>	
...
<!-- we are still in the itemised or consulting loop -->
    ...
    {section name=line start=0 loop=$invoice.tax_grouped step=1}
    	...
    	@if(($invoice.tax_grouped[line].tax_amount == "0") ) {php}break;{/php} @endif
