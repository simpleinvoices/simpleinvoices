...
@if($smarty.post.expense_account_id != "" && $smarty.post.id != null ) 
	@include('../extensions/expense/templates/default/expense/save.tpl')
@else
	...
	<tr>
		...
		<select name="customer_id">
			...
			@foreach(from=$expense_add.customer_all item=customer)
				<option @if($biller.id == $defaults.customer) selected @endif value="{{ $customer['id'] }}">{{ $customer['name'] }}</option>
			@endforeach
		</select>
		</td>
	</tr>
	...
	<tr>
        ...
        <tr>
            <td class="details_screen">
              {{ $LANG['tax'] ?? '' }} @if($defaults.tax_per_line_item > 1){$smarty.section.tax.index+1}@endif
             </td>
           <td>
            <select 
            ...
            @foreach(from=$taxes item=tax)
                <option @if($tax.tax_id == $defaults.tax AND $smarty.section.tax.index == 0) selected @endif   value="{{ $tax['tax_id'] }}">{{ $tax['tax_description'] }}</option>
            @endforeach
        </select>
        </td>
        </tr>
