<form name="frmpost" action="index.php?module=expense&view=save&id={$smarty.get.id}" method="post">
  ...
  <table class="center">
    ...
    <tr>
      ...
      <td>
        <select name="expense_account_id" class="validate[required]">
          ...
          <option {if $expense_account.id == $expense.expense_account_id}selected{/if} value="{$expense_account.id}">
    ...
    <tr>
      ...
      <td>
        <select name="biller_id" class="validate[required]">
          ...
          <option {if $biller.id == $expense.biller_id} selected {/if} value="{$biller.id}">
            {$biller.name}
          </option>
          {/foreach}
        </select>
    ...
    <tr>
      ...
      <td>
        <select name="customer_id">
         ...
         <option {if $customer.id == $expense.customer_id} selected {/if} value="{$customer.id}">
           {$customer.name}
         </option>
         {/foreach}
        </select>
    ...
    <tr>
      ...
      <td>
        <select name="invoice_id">
          ...
          <option {if $invoice.id == $expense.invoice_id} selected {/if} value="{$invoice.id}">
            {$invoice.index_name}
          </option>
          {/foreach}
        </select>
    ...
    <tr>
      ...
      <td>
        <select name="product_id">
          ...
          <option {if $product.id == $expense.product_id} selected {/if} value="{$product.id}">
            {$product.description}
          </option>
          {/foreach}
        </select>
          ...
          <tr>
            ...
            <td>
              <select id="tax_id[0][{$smarty.section.tax.index}]" name="tax_id[0][{$smarty.section.tax.index}]">
                ...
                <option {if $tax.tax_id === $detail.expense_tax.$index.tax_id} selected {/if} value="{$tax.tax_id}">
    ...
    <tr>
      <td class="details_screen">{$LANG.notes}</td>
      <td>
        <textarea class="editor" name='note' rows="8" cols="50">
          {$expense.note|unescape}
        </textarea>
      </td>
    </tr>
  </table>
