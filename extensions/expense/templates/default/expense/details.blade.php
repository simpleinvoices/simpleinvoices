	...
	<tr>
		...
		<select name="expense_account_id" class="validate[required]">
			...
			@foreach(from=$detail.expense_account_all item=expense_account)
				<option @if($expense_account.id == $expense.expense_account_id) selected @endif value="{{ $expense_account['id'] }}">{{ $expense_account['name'] }}</option>
			@endforeach
		</select>
		</td>
	</tr>
	...
	<tr>
		...
		<select name="biller_id" class="validate[required]">
			...
			@foreach(from=$detail.biller_all item=biller)
				<option @if($biller.id == $expense.biller_id) selected @endif value="{{ $biller['id'] }}">{{ $biller['name'] }}</option>
			@endforeach
		</select>
		</td>
	</tr>
	<tr>
		...
		<select name="customer_id">
			...
			@foreach(from=$detail.customer_all item=customer)
				<option @if($customer.id == $expense.customer_id) selected @endif value="{{ $customer['id'] }}">{{ $customer['name'] }}</option>
			@endforeach
		</select>
		</td>
	</tr>
	<tr>
		...
		<select name="invoice_id">
			...
			@foreach(from=$detail.invoice_all item=invoice)
				<option  @if($invoice.id == $expense.invoice_id) selected @endif value="{{ $invoice['id'] }}">{{ $invoice['index_name'] }}</option>
			@endforeach
		</select>
		</td>
	</tr>
	<tr>
		...
		<select name="product_id">
			...
			@foreach(from=$detail.product_all item=product)
				<option @if($product.id == $expense.product_id) selected @endif value="{{ $product['id'] }}">{{ $product['description'] }}</option>
			@endforeach
		</select>
		</td>
	</tr>
						...
						{section name=tax start=0 loop=$defaults.tax_per_line_item step=1}
							<td>				                				                
								...
								@foreach(from=$taxes item=tax)
									<option @if($tax.tax_id === $detail.expense_tax.$index.tax_id) selected @endif value="{{ $tax['tax_id'] }}">{{ $tax['tax_description'] }}</option>
								@endforeach
							</select>
							</td>
						{/section}
