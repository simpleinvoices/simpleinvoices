...
<div class="welcome">
  <form name="frmpost" action="index.php?module=reports&amp;view=custom_field_3" method="post">
    <table class="center">
      <tr>
        <td class="details_screen">Sales Rep</td>
        <td>
          {if $cf3 == null }
          ...
          {foreach from=$cf3 item=list_biller}
            <option {if $list_biller.id == $biller_id}selected{/if} value="{$list_biller.custom_field3}">
              {$list_biller.custom_field3}
            </option>
          {/foreach}
          </select>
          {/if}
      ...
      <tr>
        <td class="details_screen" >End date</td>
        <td>
          <input type="text" class="validate[required,custom[date],length[0,10]] date-picker"
...
{if $smarty.post.submit != null OR $view == export}
  ...
  <table width="100%">
    <tr>
      <td width="75%" align="left">
        <strong>Sales Rep:</strong> {$custom_field3}
        <br />
        <br />
      </td>
      <td width="25%">
  ...
  <table class="center" width="100%">
    ...
    {section name=invoice loop=$invoices}
      ...
      {/if}
      <tr>
        <td class="details_screen">{$index} {$invoices[invoice].id}</td>
        <td>&nbsp;&nbsp;</td>
