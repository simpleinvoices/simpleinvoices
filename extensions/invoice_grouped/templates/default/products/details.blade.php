	...
	<tr>
		...
		<select name="default_tax_id">
			@foreach(from=$taxes item=tax)
				<option value="{{ $tax['tax_id'] }}" @if($tax.tax_id == $product.default_tax_id)selected@endif>{{ $tax['tax_description'] }}</option>
			@endforeach
		</select>
		</td>
	</tr>
	<tr>
		...
		<select name="custom_field1">
			...
			@foreach(from=$product_group item=pg)
				<option value="{{ $pg['name'] }}" @if($pg.name == $product.custom_field1)selected@endif>{{ $pg['name'] }}</option>
			@endforeach
		</select>
		</td>
	</tr>
