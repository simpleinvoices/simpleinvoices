...
function ts_resortTable(lnk) {
    ...
    for (j=firstNonSortedRow;j<table.rows.length;j++) {
		nonSortedRows[nonSortedRows.length] = table.rows[j];
	}

    for (j=getHeaderRow(table)+1;j<table.rows.length;j++) {
    ...
    for (i=0; i<nonSortedRows.length; i++) {
...
function ts_makeSortable(table) {
	if (table.rows && table.rows.length > 0) {
        var firstRow = table.rows[getHeaderRow(table)];
    }
    if (!firstRow) return;
...
var SORT_COLUMN_INDEX;
 ...
 document.write("Some features have been disabled because your using the Internet Explorer browser. Please use <a href=\"http://www.getfirefox.com\">Firefox</a> to enable all the features of Simple Invoices")
}
