	...
	@foreach(($invoiceItems ?? []) as $invoiceItem)
			...
			<tr class="tbl1-left tbl1-right tbl1-bottom">
				...
				<td class="tbl1-bottom">{{ ($preference['pref_currency_sign'] ?? '')|si_currency_display }} {{ $invoiceItem['unit_price'] ?? '' | siLocal_number }}</td>
				<td class="tbl1-right tbl1-bottom" align="right">{{ ($preference['pref_currency_sign'] ?? '')|si_currency_display }} {{ $invoiceItem['total'] ?? '' | siLocal_number }}</td>
			</tr>
...
@php $invoice_gross_total = ($invoice['total'] ?? 0) - ($invoice['total_tax'] ?? 0); @endphp
		<tr>
			...
			<td align="right">{{ ($preference['pref_currency_sign'] ?? '')|si_currency_display }} {{ $invoice_gross_total | siLocal_number }}</td>
		</tr>	
