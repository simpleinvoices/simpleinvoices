		...
		<table class="table table-vcenter table-hover card-table">
			...
			@foreach($data as $i => $biller)
				...
				<tr>
					...
					<td class="text-end fw-semibold">{!! CurrencySignHelper::format($biller['sum_total'] ?? 0, $biller['currency_sign'] ?? '', '', $biller['currency_code'] ?? '') !!}@if(!empty($biller['currency_code'])) <span class="badge bg-secondary-lt text-muted ms-1" style="font-size:.7rem">{{ $biller['currency_code'] }}</span>@endif</td>
