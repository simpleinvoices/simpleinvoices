   ...
   <table>
    ...
    <tr>
      ...
      <td class="">
        ...
        <tr><th>&nbsp;</th></tr> <!-- Here to add space so calendar shows -->
  </table>
...
<div class="si_form" id="si_statement_info">
  <div class="si_statement_info1">
    <table>
      <tr>
        <th>{$LANG.biller}:</th>  <td>{$biller_details.name|htmlsafe}</td>
      </tr>
      <tr>
        <th>{$LANG.customer}:</th>  <td>{$customer_details.name|htmlsafe}</td>
      </tr>
    </table>
  </div>
...
{if $filter_by_date == "yes"} 
  <div><strong>{$LANG.statement_for_the_period} {$start_date|htmlsafe} {$LANG.to_lowercase} {$end_date|htmlsafe}</strong></div>
<br />
{/if}
  ...
  <table class="center" width="100%">
    ...
    <tbody>
     {section name=invoice loop=$invoices}
      ...
      {/if}
      <tr>
        <td class="si_right">{$index|htmlsafe}
