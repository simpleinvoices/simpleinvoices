<script type="text/javascript">
		...
		siTablerGrid('#manageGrid', {
			...
			colModel : [
				...
				{display: @json($LANG['description'] ?? ''), name : 'tax_description', width : 60 * percentage_width, sortable : true, align: 'left'},
				{display: @json($LANG['rate'] ?? ''), name : 'tax_percentage', width : 20 * percentage_width, sortable : true, align: 'right'},
			...
			searchitems : [
				{display: @json($LANG['description'] ?? ''), name : 'tax_description', isdefault: true},
				{display: @json($LANG['tax_percentage'] ?? ''), name : 'tax_percentage'}
			],
