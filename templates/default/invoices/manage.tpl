...
{foreach from=$invoices item=invoice}
	...
	href="index.php?module=invoices&view=quick_view&invoice={$invoice.id}&type={$invoice.type_id}">
	...
	href="index.php?module=invoices&view=details&invoice={$invoice.id}&action=view&type={$invoice.type_id}">
	...
	href="index.php?module=invoices&view=templates/template&invoice={$invoice.id}&action=view&location=print&type={$invoice.type_id}">
	<img src="images/common/printer.gif" height="16" border="-5px" padding="-4px" valign="bottom" /><!-- print --></a>
 
	<!-- EXPORT TO PDF -->
	<a title="{$LANG.export_tooltip} {$invoice.pref_inv_wording} {$invoice.id} {$LANG.export_pdf_tooltip}"
	class="index_table" href="{$invoice.url_for_pdf}"><img src="images/common/pdf.jpg" height="16" border="-5px" padding="-4px" valign="bottom" /><!-- pdf --></a>

	<!--XLS -->
	<a title="{$LANG.export_tooltip} {$invoice.pref_inv_wording}{$invoice.id} {$LANG.export_xls_tooltip} {$spreadsheet} {$LANG.format_tooltip}"
	 class="index_table" href="index.php?module=invoices&view=templates/template&invoice={$invoice.id}&action=view&type={$invoice.type_id}&location=print&export={$spreadsheet}">
	 <img src="images/common/xls.gif" height="16" border="0" padding="-4px" valign="bottom" /><!-- $spreadsheet --></a>

	<!-- DOC -->
	<a title="{$LANG.export_tooltip} {$invoice.pref_inv_wording} {$invoice.id} {$LANG.export_doc_tooltip} {$word_processor} {$LANG.format_tooltip}"
	class="index_table" href="index.php?module=invoices&view=templates/template&invoice={$invoice.id}&action=view&type={$invoice.type_id}&location=print&export={$word_processor}">
	<img src="images/common/doc.png" height="16" border="0" padding="-4px" valign="bottom" /><!-- $word_processor --></a>

	<!-- Payment -->
	<a title="{$LANG.process_payment} {$invoice.pref_inv_wording} {$invoice.id}"
	class="index_table" href="index.php?module=payments&view=process&invoice={$invoice.id}&op=pay_selected_invoice">$</a>

	<!-- Email -->
	<a href="index.php?module=invoices&view=email&stage=1&invoice={$invoice.id}" title="{$LANG.email}  {$invoice.pref_inv_wording} {$invoice.id}">
