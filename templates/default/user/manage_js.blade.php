<script type="text/javascript">
		...
		siTablerGrid('#manageGrid', {
			...
			colModel : [
				...
				{display: @json($LANG['name'] ?? ''), name : 'name', width : 25 * percentage_width, sortable : true, align: 'left'},
				{display: @json($LANG['email'] ?? ''), name : 'email', width : 35 * percentage_width, sortable : true, align: 'left'},
				{display: @json($LANG['role'] ?? ''), name : 'role', width : 25 * percentage_width, sortable : true, align: 'left'},
			...
			searchitems : [
				...
				{display: @json($LANG['role'] ?? ''), name : 'ur.name'}
			],
